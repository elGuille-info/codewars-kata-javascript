...
All table and column names will be simple strings with no spaces. 
    Constant strings will be written in normal SQL format, with single quotes and escaping internal single quotes by doubling them (for example, 'a ''string'' containing quotes'). 
    For values, Haskell will use string constants only; numerical strings will, when necessary, have leading zeroes for correct ordering.
...
The database will be a simple object (read 'association list' for Haskell) passed to the constructor of your SQL engine. 
    Each table will be a property in the object, with the table name as the property key. 
    The tables will be arrays of objects; each object is a row, and each property on the object is a column in that row. 
    (This isn't the most efficient possible structure, but it's easy to work with.) 
    An example database is provided with the test cases.

Results
Your execute method must return an array of objects. 
    Each object must have a property for each column-id in the SELECT clause of the query, set to the selected value for that column.

Test Helpers
Because we are not implementing ORDER BY, and the built-in Test.assertSimilar doesn't work correctly with arrays of objects when the objects are in different orders, 
    I have provided a assertSimilarRows method which does the same thing as Test.assertSimilar but works with the objects your execute method will return.
...
SELECT
A SELECT clause dictates which columns from which tables each row should contain. 
    For example, the clause SELECT movies.title, directors.name gives each row two columns: 
    movies.title would take the title value from the movies table, and directors.name would take the name value from the directors table. 
    Only tables included in the query using FROM and JOIN clauses can be selected from. 
    (NOTE: SELECT * will not be used in this kata.)

FROM
A FROM clause identifies the primary table to start from. 
    For example, the clause FROM movies would include the movies table in the query. 
    This allows the movies table to be read by the SELECT clause.

JOIN
Any number of JOIN clauses (including 0; JOIN clauses are not required for a valid query) 
    may define other related tables that should be included, along with a conditional statement (see below) dictating how the tables relate to one another. 
    For example JOIN directors ON directors.ID = movies.directorID would link the table directors with the table movies, 
...
If any row in either table could match multiple times, multiple rows should be included in the results. 
    For this kata, if a row does not have a matching row in a related table, that row should be omitted from the results entirely. 
...
An optional WHERE clause may describe a conditional statement (see below) that dictates which rows will be returned. 
    If the conditional statement is true for a particular row, it will be returned. 
    If it is not true, the row will be omitted from the results.

Conditional Statements
Conditional statements have two values and a comparison operator. 
    For example, [value1] >= [value2] will be true if value1 is greater than or equal to value2, and false if not. 
    These values can be one of two things:

A column value, referring to a table used by the query. 
    For example, movies.ID = actors_in_movies.movieID will compare a movies row's ID value to an actors_in_movies row's movieID value.
A constant value, either a number or a string for this kata (Haskell: string only). 
    A number will simply be a series of digits, perhaps with decimals. 
    For example, 101 and 1.01 are both numerical contants. 
    A string will be a number of characters wrapped in single-quotes. 
    To represent literal single-quote characters within a string, two single-quotes will be used. 
    For example, 'string' and 'string with ''single quotes'' embedded' are both string constants.
Normally, one side or the other will be a column value, and the other will be a column value or a constant. 
    The comparison operator will be one of the following: = (equal), < (less than) <= (less than or equal to), > (greater than), >= (greater than), <> (not equal).
...
function assertSimilarRows(select, resOK) {
    console.log(select + " ?= " + resOK)
    if (select != resOK) {
